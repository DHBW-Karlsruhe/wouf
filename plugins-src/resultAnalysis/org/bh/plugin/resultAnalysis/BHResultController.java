...
public class BHResultController extends OutputController {
	...
	/**
	 * chart keys for result to chart mapping used in
	 * <code>BH_APV_ResultPanel>/code>
	...
	public void setResult(Map<String, Calculable[]> result, DTOScenario scenario) {
		...
		if (scenario.getDCFMethod().getUniqueId().equals("apv")) {
			...
			apv_panel.setBorder(BHBorderFactory.getInstacnce()
					.createTitledBorder(
							BHBorderFactory.getInstacnce().createEtchedBorder(
									EtchedBorder.LOWERED), "apv"));
			BH_FCF_ResultPanel fcf_panel = new BH_FCF_ResultPanel(true);
			fcf_panel.setBorder(BorderFactory.createTitledBorder(
					BorderFactory.createEtchedBorder(EtchedBorder.LOWERED),
					translator.translate("fcf")));
			BH_FTE_ResultPanel fte_panel = new BH_FTE_ResultPanel(true);
			fte_panel.setBorder(BorderFactory.createTitledBorder(
					BorderFactory.createEtchedBorder(EtchedBorder.LOWERED),
					translator.translate("fte")));
		...
		if (scenario.isIntervalArithmetic()) {
			...
			for (String chartKey : super.view.getBHchartComponents().keySet()) {
				...
				if (chartKey.equals(ChartKeys.APV_WF_SV.toString())) {
					...
					comp.addValue(
							result.get("org.bh.plugin.apv.APVCalculator$Result.PRESENT_VALUE_FCF")[0]
									.getMin(),
					...
					comp.addValue(
							result.get("org.bh.plugin.apv.APVCalculator$Result.PRESENT_VALUE_TAX_SHIELD")[0]
									.getMin(),
							translator.translate(ChartKeys.APV_WF_SV),
							translator
					...
					comp.addValue(
							...
							translator
									...
									+ " " + translator.translate("max"));
					comp.addValue(
							result.get("org.bh.calculation.IShareholderValueCalculator$Result.DEBT")[0]
									.getMin()
									* -1,
							translator.translate(ChartKeys.APV_WF_SV),
							translator
					...
					IBHAddGroupValue comp = (IBHAddGroupValue) super.view
							.getBHchartComponents().get(
					...
					for (int i = 0; i < length; i++) {
						String name = scenario.getChildren().get(i)
								.get(DTOPeriod.Key.NAME).toString();
						comp.addValue(
								result.get("org.bh.plugin.apv.APVCalculator$Result.PRESENT_VALUE_FCF")[i]
										.getMin(),
								translator
										.translate("org.bh.plugin.apv.APVCalculator$Result.PRESENT_VALUE_FCF")
										+ " " + translator.translate("min"),
								name, IBHAddGroupValue.MIN_POS);
						comp.addValue(
								result.get("org.bh.calculation.IShareholderValueCalculator$Result.DEBT")[i]
										.getMin(),
								translator
										.translate("org.bh.calculation.IShareholderValueCalculator$Result.DEBT")
										+ " " + translator.translate("min"),
					...
					comp.addValue(
							result.get("org.bh.plugin.fcf.FCFCalculator$Result.TOTAL_CAPITAL")[0]
									.getMin(),
					...
					if (result
						...
						String name = scenario.getChildren().get(0)
								.get(DTOPeriod.Key.NAME).toString();
						comp.addValue(
								result.get("org.bh.calculation.IShareholderValueCalculator$Result.FREE_CASH_FLOW")[0]
										.getMin(),
								translator.translate(ChartKeys.FCF_BC_FCF
										.toString())
										+ " "
					...
					for (int i = 1; i < scenario.getChildrenSize(); i++) {
						String name = scenario.getChildren().get(i)
								.get(DTOPeriod.Key.NAME).toString();
						comp.addValue(
								result.get("org.bh.calculation.IShareholderValueCalculator$Result.FREE_CASH_FLOW")[i]
										.getMin(),
								translator.translate(ChartKeys.FCF_BC_FCF
										.toString())
										+ " "
					...
					for (int i = 0; i < scenario.getChildrenSize(); i++) {
						String name = scenario.getChildren().get(i)
								.get(DTOPeriod.Key.NAME).toString();
						comp.addValue(
								result.get("org.bh.plugin.fcf.FCFCalculator$Result.EQUITY_RETURN_RATE_FCF")[i]
										.getMin() * 100,
								translator
								...
								name);
						comp.addValue(
								result.get("org.bh.calculation.IShareholderValueCalculator$Result.DEBT_RETURN_RATE")[0]
										.getMin() * 100,
								translator
					...
					for (int i = 1; i < scenario.getChildrenSize(); i++) {
						String name = scenario.getChildren().get(i)
								.get(DTOPeriod.Key.NAME).toString();
						comp.addValue(
								result.get("org.bh.plugin.fte.FTECalculator$Result.FLOW_TO_EQUITY")[i]
										.getMin(),
								translator
					...
					IBHAddValue comp = super.view.getBHchartComponents().get(
							ChartKeys.FTE_BC_RR.toString());
					for (int i = 1; i < scenario.getChildrenSize(); i++) {
						String name = scenario.getChildren().get(i)
								.get(DTOPeriod.Key.NAME).toString();
						comp.addValue(
								result.get("org.bh.plugin.fte.FTECalculator$Result.EQUITY_RETURN_RATE_FTE")[i]
										.getMin() * 100,
								translator
								...
								name);
						comp.addValue(
								((Calculable) scenario.get(DTOScenario.Key.RFK))
										.getMin() * 100,
								translator
										.translate("org.bh.calculation.IShareholderValueCalculator$Result.DEBT")
										+ " " + translator.translate("min"),
				...
				if (chartKey.equals(ChartKeys.APV_WF_SV.toString())) {
					...
					for (int i = 0; i < scenario.getChildrenSize(); i++) {
						String name = scenario.getChildren().get(i)
								.get(DTOPeriod.Key.NAME).toString();
						comp.addValue(
								result.get("org.bh.plugin.fte.FTECalculator$Result.PRESENT_VALUE_TAX_SHIELD")[i]
										.toNumber(),
								translator
	...
	public static Map<String, Calculable> getFormulaMap(DTOScenario scenario,
			Map<String, Calculable[]> result, int t) {
		...
		if (t == scenario.getChildrenSize() - 1) {
			...
			putFormulaValue(
					...
					"FCFT",
					result,
					"org.bh.calculation.IShareholderValueCalculator$Result.FREE_CASH_FLOW",
					t);
			putFormulaValue(
					formulaMap,
					"FKT",
					result,
					"org.bh.calculation.IShareholderValueCalculator$Result.DEBT",
					t);
			...
			putFormulaValue(
					...
					"UWAPV,T",
					result,
					"org.bh.calculation.IShareholderValueCalculator$Result.SHAREHOLDER_VALUE",
					t);
			...
			putFormulaValue(
					...
					"rvTEK",
					result,
					"org.bh.plugin.fcf.FCFCalculator$Result.EQUITY_RETURN_RATE_FCF",
					t);
			...
			putFormulaValue(
					...
					"org.bh.calculation.IShareholderValueCalculator$Result.SHAREHOLDER_VALUE",
					t);
			putFormulaValue(formulaMap, "FTET", result,
					"org.bh.plugin.fte.FTECalculator$Result.FLOW_TO_EQUITY", t);
			putFormulaValue(
					formulaMap,
					"rvTEK",
					result,
					"org.bh.plugin.fte.FTECalculator$Result.EQUITY_RETURN_RATE_FTE",
					t);
		} else {
			...
			putFormulaValue(
					...
					"org.bh.calculation.IShareholderValueCalculator$Result.SHAREHOLDER_VALUE",
					t);
			putFormulaValue(formulaMap, "Vu,t+1", result,
					"org.bh.plugin.apv.APVCalculator$Result.PRESENT_VALUE_FCF",
					t + 1);
			putFormulaValue(
					formulaMap,
					"Vs,t+1",
					result,
					"org.bh.plugin.apv.APVCalculator$Result.PRESENT_VALUE_TAX_SHIELD",
					t + 1);
			...
			putFormulaValue(
					...
					"org.bh.calculation.IShareholderValueCalculator$Result.SHAREHOLDER_VALUE",
					t);
			putFormulaValue(formulaMap, "GKt+1", result,
					"org.bh.plugin.fcf.FCFCalculator$Result.TOTAL_CAPITAL",
					t + 1);
			putFormulaValue(
					formulaMap,
					"rvt+1EK",
			...
			putFormulaValue(
					...
					"org.bh.calculation.IShareholderValueCalculator$Result.SHAREHOLDER_VALUE",
					t + 1);
			putFormulaValue(formulaMap, "FTEt+1", result,
					"org.bh.plugin.fte.FTECalculator$Result.FLOW_TO_EQUITY",
					t + 1);
			putFormulaValue(
					formulaMap,
					"rvt+1EK",
		...
		// General
		putFormulaValue(formulaMap, "s", result,
				"org.bh.calculation.IShareholderValueCalculator$Result.TAXES",
				0);
		putFormulaValue(
				formulaMap,
				"rFK",
				result,
				"org.bh.calculation.IShareholderValueCalculator$Result.DEBT_RETURN_RATE",
				0);
		putFormulaValue(
				formulaMap,
				"ruEK",
				result,
				"org.bh.calculation.IShareholderValueCalculator$Result.EQUITY_RETURN_RATE",
				0);
	...
	protected static void putFormulaValue(Map<String, Calculable> formulaMap,
			String formulaKey, Map<String, Calculable[]> resultMap,
			String resultKey, int resultIndex) {
		...
		try {
			...
			if (e.getSource() instanceof JComboBox) {	
				...
				if (fp != null) {
					if (scenario.getDCFMethod().getUniqueId().equals("apv")) {
						if (t == scenario.getChildrenSize() - 1) {
							fp.setFormula(ff.createFormula("apv_t", getClass()
									.getResourceAsStream("APV_SHV_T.xml"),
									false));
							fp.setValues(getFormulaMap(scenario, result, t));
						} else {
							fp.setFormula(ff.createFormula("apv_T", getClass()
									.getResourceAsStream("APV_SHV_t1.xml"),
									false));
							fp.setValues(getFormulaMap(scenario, result, t));
						}
					} else if (scenario.getDCFMethod().getUniqueId()
							.equals("fcf")) {
						if (t == scenario.getChildrenSize() - 1) {
							fp.setFormula(ff.createFormula("fcf_T", getClass()
									.getResourceAsStream("FCF_SHV_T.xml"),
									false));
							fp.setValues(getFormulaMap(scenario, result, t));
						} else {
							fp.setFormula(ff.createFormula("fcf_t", getClass()
									.getResourceAsStream("FCF_SHV_t1.xml"),
									false));
							fp.setValues(getFormulaMap(scenario, result, t));
						}
					} else if (scenario.getDCFMethod().getUniqueId()
							.equals("fte")) {
						if (t == scenario.getChildrenSize() - 1) {
							fp.setFormula(ff.createFormula("fte_T", getClass()
									.getResourceAsStream("FTE_SHV_T.xml"),
									false));
							fp.setValues(getFormulaMap(scenario, result, t));
						} else {
							fp.setFormula(ff.createFormula("fte_t", getClass()
									.getResourceAsStream("FTE_SHV_t1.xml"),
									false));
							fp.setValues(getFormulaMap(scenario, result, t));
						}
					}
				...
				if (b.getKey().toString()
						.equals(BHResultPanel.Keys.EXPORTSCENARIO.toString())) {
				...
				} else if (b.getKey().toString()
						.equals(BHResultPanel.Keys.PRINTSCENARIO.toString())) {
					...
					((IPrint) pPlugs.values().toArray()[0])
							.printScenarioResults(scenario, result, charts);
				}
			}
